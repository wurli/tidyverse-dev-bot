...
-   !begin-bullet!
    bslib now uses the `{brand.yml}` package for reading brand.yml
    files. This change improves compatibility with other tools that use
    brand.yml files, such as Quarto. The `{brand.yml}` package must be
    installed for `_brand.yml` files to be automatically discovered.
    (#1227)
    !end-bullet!
