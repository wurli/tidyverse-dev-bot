...
-   !begin-bullet!
    ...
    automatically execute `pkgload::load_all()` to load the local R
    package's source code. Installing the package before testing is no
    longer required! (#402).

    !end-bullet!
-   !begin-bullet!
    Added support for function execution for `AppDriver$new(app_dir=)`.
    You must run the App or document yourself. Similar to `{mirai}`, all
    packages must be `library()`'d or `require()`'d again as the
