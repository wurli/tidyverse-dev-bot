...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `exprs()` gains a `.named` option to auto-label its arguments
    (#267).
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `enquo()` and `enexpr()` now deal with default values correctly
    (#201).
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `UQ()`, `UQS()` and `:=` now issue an error when called directly.
    The previous definitions caused surprising results when the
    operators were invoked in wrong places (i.e. not in quasiquoted
    arguments).
...
-   !begin-bullet!
    ...
    `!!`. If `!!` and `!!!` were native R operators, they would parse to
    the functional calls `` `!!`() `` and `` `!!!`() ``. This is now the
    preferred way to unquote with a function call rather than with the
    operators. We haven't decided yet whether we will deprecate `UQ()`
    and `UQS()` in the future. In any case we recommend using the new
    functional forms.
...
-   !begin-bullet!
    ...
    return type of a function. In addition the new functions require
    their callers to explicitly supply an environment for the quosures.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    The `new_call()` constructor creates calls out of a callable object
    (a function or an expression) and a pairlist of arguments. It is
    useful to avoid costly internal coercions between lists and
