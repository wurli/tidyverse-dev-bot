...
-   !begin-bullet!
    ...
    which of `\dontrun{}`, `\dontshow{}`, `\donttest{}` and
    `\testonly{}` are commented out. The `strict` parameter has been
    removed since it is no longer necessary because `load_all` can
    respect namespaces. (Fixes #118)

    !end-bullet!
-   !begin-bullet!
    `build()`, `check()`, `install()` etc now run R in `--vanilla` mode
    ...
    environment in which the package is run between your computer and
    other computers (e.g. the CRAN server) (Fixes #145)

    !end-bullet!
-   !begin-bullet!
    All system R command now print the full command used to make it
...
-   !begin-bullet!
    ...
    not exist, isn't a directory, or doesn't contain a `DESCRIPTION`
    file.

    !end-bullet!
-   !begin-bullet!
    New function `inst()` takes the name of a package and returns the
    installed path of that package. (Winston Chang. Fixes #130). This
    makes it possible to use `devtools` functions (e.g. `unload`) with
    regular installed packages, not just in-development source packages.

    !end-bullet!
-   !begin-bullet!
    New function `devtest()` returns paths to an internal testing
...
-   !begin-bullet!
    ...
    `<namespace:xxxx>`{=html}, and then the objects namespace are copied
    to the package environment, `<package:xxxx>`{=html}. This more
    accurately simulates how packages are normally loaded. However, all
    of the objects (not just the exported ones) are still copied to the
    package environment. (Winston Chang. Fixes #3, #60, and #125)

    !end-bullet!
-   !begin-bullet!
    Packages listed in Imports and Depends are now loaded into an
    imports environment, with name attribute "imports:xxxx", which is
    the parent of the namespace environment. The imports environment is
    in turn a child of the `<namespace:base>`{=html} environment, which
    is a child of the global environment. This more accurately simulates
    how packages are normally loaded. These packages previously were
    loaded and attached. (Winston Chang. Fixes #85)
...
-   !begin-bullet!
    ...
    by creating a copy of the devtools namespace environment, and
    calling `load_all` from that environment. (Winston Chang)

    !end-bullet!
-   !begin-bullet!
    The `.onLoad` and `.onAttach` functions for a development package
    are now both called when loading a package for the first time, or
    with `reset=TRUE`, and the order more correctly simulates normal
    package loading (create the namespace, call `.onLoad`, copy objects
    to the package environment, and then call `.onAttach`). (Winston
    Chang)

    !end-bullet!
-   !begin-bullet!
    `load_all` will now throw a warning if a dependency package does not
    satisfy the version requirement listed in DESCRIPTION. (Winston
    Chang. Fixes #109)
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `load_all` now has an option `export_all`. When set to TRUE, only
    the objects listed as exports in NAMESPACE are exported. (Winston
    Chang)

    !end-bullet!
-   !begin-bullet!
    `load_all` now compiles C files in the /src directory. (Winston
    Chang)

    !end-bullet!
-   !begin-bullet!
    New functions `compile_dll()` and `clean_dll()`, which compile
    C/C++/ Fortran source code, and clean up the compiled objects,
    respectively. (Winston Chang. Fixes #131)
...
-   !begin-bullet!
    New function `loaded_packages()`, which returns the names of
    packages that are loaded and attached.

    !end-bullet!
-   !begin-bullet!
    Packages loaded with `load_all` now store devtools metadata in their
    namespace environment, in a variable called `.__DEVTOOLS__`. This
    can be accessed with the `dev_meta` function. (Winston Chang. Fixes
    #128)
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    New function `parenvs()`, which parents the parent environments of
    an object. (Winston Chang)
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    All remote install functions give better error messages in the case
    of http errors (Fixes #82).

    !end-bullet!
-   !begin-bullet!
    `install` has new quick option to make package installation faster,
    by sacrificing documentation, demos and multi-architecture binaries.
    (Fixes #77)

    !end-bullet!
-   !begin-bullet!
    `install_url`, `install_github` and `install_gitorious` gain a
    subdir argument which makes it possible to install packages that are
...
-   !begin-bullet!
    `with_debug` function temporarily sets env vars so that compilation
    is performed with the appropriate debugging flags set. Contributed
    by Andrew Redd.
...
-   !begin-bullet!
    ...
    `R CMD check`. (Fixes #78)

    !end-bullet!
-   !begin-bullet!
    `check_cran` has received a massive overhaul: it now checks multiple
    packages, installs dependencies (in user specified library), and
    parse check output to extract errors and warnings

    !end-bullet!
-   !begin-bullet!
    `check` uses new `--as-cran` option to make checking as close to
    CRAN as possible (fixes #68)
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    on windows, devtools now looks in the registry to find where Rtools
    is installed, and does a better a job of locating gcc. (Contributed
    by Andrew Redd)
...
-   !begin-bullet!
    ...
    `with_locale`, `with_options`, `with_path`, ... (Fixes #89)

    !end-bullet!
-   !begin-bullet!
    `release` ask more questions and randomises correct answers so you
    really need to read them (Fixes #79)
...
-   !begin-bullet!
    `decompress` now uses target directory as defined in the function
    call when expanding a compressed file. (Fixes #84)

    !end-bullet!
-   !begin-bullet!
    `document` is always run in a C locale so that `NAMESPACE` sort
    order is consistent across platforms.

    !end-bullet!
-   !begin-bullet!
    `install` now quotes `libpath` and build path so paths with embedded
    spaces work (Fixes #73 and #76)
...
-   !begin-bullet!
    ...
    arguments on to `R CMD install` (replaces `...` which didn't
    actually do anything). (Fixes #69)
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `missing_s3` function to help figure out if you've forgotten to
    export any s3 methods
...
-   !begin-bullet!
    ...
    current environment), but ensures that each example works
    standalone.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    all external package installation functions are vectorised so you
    can install multiple packages at time

    !end-bullet!
-   !begin-bullet!
    new `install_gitorious` function install packages in gitorious
    repos.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    include `install_version` function from Jeremy Stephens for
    installing a specific version of a CRAN package from the archive.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    check to see if Rtools is already available before trying to mess
    with the paths. (Fixes #55)
...
-   !begin-bullet!
    ...
    automatically cleared so that all files are loaded again next time
    you try (Fixes #55)

    !end-bullet!
-   !begin-bullet!
    functions that run R now do so with `R_LIBS` set to the current
    `.libPaths()` - this will ensure that checking uses the development
    library if you are in development mode. `R_ENVIRON_USER` is set to
    an empty file to avoid your existing settings overriding this.

    !end-bullet!
-   !begin-bullet!
    `load_data` (called by `load_all`) will also load data defined in R
    files in the data directory. (Fixes #45)
...
-   !begin-bullet!
    external R commands always run in locale `C`, because that's what
    the CRAN severs do.
...
-   !begin-bullet!
    ...
    environment. Optionally (`vanilla = T`), it will source in a vanilla
    R environment which ignores all local environment settings.
...
-   !begin-bullet!
    ...
    whether a binary or a source version (with no vignettes or manuals)
    is built. Confusingly, binary packages are built with
    `R CMD INSTALL`.

    !end-bullet!
-   !begin-bullet!
    `build_win` sends your package to the R windows builder, allowing
    you to make a binary version of your package for windows users if
    you're using linux or macOS (if you're using windows already, use
    `build(binary = T)`)
...
-   !begin-bullet!
    if using `.Rpackages` config file, default function is used last,
    not first.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `document` uses `roxygen2` instead of `roxygen`. It now loads
    package dependency so that they're available when roxygen executes
...
-   !begin-bullet!
    new `bash()` function that starts bash shell in package directory.
    Useful if you want to use git etc.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    Quote the path to R so it works even if there are spaces in the
    path.
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    `install` now works correctly with `devel_mode` to install packages
    in your development library
...
-   !begin-bullet!
    ...
    !end-bullet!
-   !begin-bullet!
    All `R CMD xxx` functions now use the current R, not the first R
